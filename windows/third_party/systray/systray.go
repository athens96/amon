...
// SetOnClick registers the callback invoked when the tray icon is left-clicked.
// Right-click continues to open the native context menu.
func SetOnClick(callback func()) {
	trayClickLock.Lock()
	trayClick = callback
	trayClickLock.Unlock()
}

// SetOnRightClick registers the callback invoked when the tray icon is right-clicked.
// When unset, right-click continues to open the native context menu.
func SetOnRightClick(callback func()) {
	trayClickLock.Lock()
	trayRightClick = callback
	trayClickLock.Unlock()
}

func invokeTrayClick() bool {
	trayClickLock.RLock()
	callback := trayClick
	trayClickLock.RUnlock()
	if callback == nil {
		return false
	}
	go callback()
...
func invokeTrayRightClick() bool {
	trayClickLock.RLock()
	callback := trayRightClick
	trayClickLock.RUnlock()
	if callback == nil {
		return false
	}
