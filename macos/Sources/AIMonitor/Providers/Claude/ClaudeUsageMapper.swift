...
enum ClaudeUsageMapper {
    ...
    static func planFromProfile(_ response: HTTPResponse) -> String? {
        guard (200..<300).contains(response.statusCode),
              let body = ProviderParse.jsonObject(response.body)
        else { return nil }
        let account = body["account"] as? [String: Any]
        let organization = body["organization"] as? [String: Any]
        ...
        } else if let lowered = tier?.lowercased() {
            // 불리언이 없어도 티어 문자열에서 유추 ("…_max_20x" / "…_pro_…").
            if lowered.contains("max") { subscriptionType = "max" }
            else if lowered.contains("pro") { subscriptionType = "pro" }
        }
        return formatPlan(subscriptionType: subscriptionType, rateLimitTier: tier)
    }
