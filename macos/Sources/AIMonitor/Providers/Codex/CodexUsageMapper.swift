...
enum CodexUsageMapper {
    ...
    static func mapUsageResponse(
        ...
        // Model-specific limits (e.g. GPT-5.3-Codex-Spark and newer Codex models) ride in a separate
        // `additional_rate_limits` array, each entry reusing the primary/secondary window shape.
        // Surface every model limit as its own meter so new Codex model launches do not need another
    ...
    /// Model-specific limits from `additional_rate_limits`. Each array entry is a named limit whose
    /// `rate_limit` reuses the primary (5-hour) / secondary (weekly) window shape, so parsing mirrors the
    /// core Session/Weekly path exactly — including the fresh-window 1%→0 normalization. A non-dictionary
    /// or null array element is skipped rather than discarding its valid siblings.
    private static func additionalRateLimitLines(body: [String: Any], now: Date) -> [MetricLine] {
        ...
        for entry in entries {
            guard let rateLimit = entry["rate_limit"] as? [String: Any],
                  let baseLabel = additionalLimitLabel(entry),
                  seenLabels.insert(baseLabel).inserted
            else {
                continue
            }
    ...
    /// Display label for a Codex model-specific limit. Prefer the explicit limit name; fall back to
    /// `metered_feature`. Keep existing Spark labels stable while making newer Codex model names readable.
    private static func additionalLimitLabel(_ entry: [String: Any]) -> String? {
        let raw = [entry["limit_name"], entry["metered_feature"]]
            .compactMap { ($0 as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) }
            .first { !$0.isEmpty }
        guard let raw else { return nil }
        return shortAdditionalLimitLabel(raw)
    }

    private static func shortAdditionalLimitLabel(_ raw: String) -> String {
        var label = raw
            .replacingOccurrences(of: "_", with: " ")
            .replacingOccurrences(of: "-", with: " ")
            .titleCased(separator: { $0 == " " })
        for prefix in ["GPT ", "Gpt ", "GPT5 ", "Gpt5 ", "Codex "] where label.hasPrefix(prefix) {
        ...
        if label.lowercased().contains("spark") { return "Spark" }
