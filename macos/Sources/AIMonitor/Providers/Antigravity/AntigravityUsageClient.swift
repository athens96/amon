...
struct AntigravityUsageClient: Sendable {
    ...
    static var googleClientSecret: String? {
        ProcessInfo.processInfo.environment["AMON_GOOGLE_OAUTH_CLIENT_SECRET"]
    }
    ...
    func refreshGoogleToken(_ refreshToken: String) async -> TokenRefreshOutcome {
        guard let url = URL(string: Self.googleOAuthURL),
              let clientID = Self.googleClientID, !clientID.isEmpty,
              let clientSecret = Self.googleClientSecret, !clientSecret.isEmpty else {
            return .unavailable
        }
        let form = [
            "client_id=\(Self.formEncoded(clientID))",
            "client_secret=\(Self.formEncoded(clientSecret))",
