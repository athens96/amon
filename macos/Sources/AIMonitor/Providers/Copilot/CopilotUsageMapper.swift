...
enum CopilotUsageMapper {
    ...
    static func map(body: [String: Any]) throws -> CopilotMappedUsage {
        ...
        guard !lines.isEmpty else {
            ...
            // 구독이 끝난 계정은 쿼터 스냅샷이 아예 없다 — 일반 "unavailable" 대신
            // 만료 상태를 그대로 알려 재시도해 봐야 소용없음을 드러낸다.
            if (body["access_type_sku"] as? String) == "subscription_ended" {
                throw CopilotUsageError.subscriptionEnded
            }
...
enum CopilotUsageError: Error, LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .subscriptionEnded:
            return "Copilot subscription has ended for this account."
        }
    }
}
