...
아일랜드에는 숫자와 아이콘을 표시하고, 사용량인지 잔여량인지는 툴팁에서 확인한다.
좌우는 더 긴 내용에 맞춰 같은 폭으로 조절하며, 노치가 없는 화면에서도 가운데
간격을 둔다. 마우스를 올리면 자주색 테두리가
빛나고, 누르면 짧게 확대된다. 시스템의 **동작 줄이기** 설정을 따르면 확대 효과를
생략한다. 노치 유무와 관계없이 화면 위쪽에 붙고 아래 모서리가 둥근 모양을 사용한다.
앱 전환·다른 창 클릭 때와 표시 중 1초 간격으로 macOS가 알려 주는 주 화면을
다시 확인한다. 표시 위치나 크기가 같으면 열어 둔 패널을 유지한다.
...
`--island-render`는 노치·일반 화면의 기본·호버·누름 상태 PNG를 만들고, `--island-smoke`는 실제
